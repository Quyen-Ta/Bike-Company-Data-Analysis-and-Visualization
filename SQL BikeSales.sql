...
--Product matrix based on Revenue share & Growth (BCG Matrix) - WIP
--Step 0. Identify the suitable time period for analysis
SELECT [Sub Category], YearMonth, SUM(Revenue) AS MonthlyRev
FROM Bikecompany..BikeSales
GROUP BY [Sub Category], YearMonth
ORDER BY [Sub Category], YearMonth;

SELECT YearMonth, COUNT(DISTINCT [Sub Category])
FROM Bikecompany..BikeSales
GROUP BY YearMonth
ORDER BY YearMonth;
/*
- Time period for this dataset is Jan15-Jul16
- However, some products were launched in Jan16 and did not have data before this point. Additionally, some products do not have data for Jul16. Therefore, we'll use Jan16-Jun16 data for this analysis
- First, we'll calculate the Rev share of each sub category
- Then, we'll calculate the monthly Rev growth rate for each sub category during this period
*/

--Step 1: Caculate Rev Share
SELECT [Sub Category],
		SUM(Revenue)/ SUM(SUM(Revenue))	OVER() AS RevShare
FROM Bikecompany..BikeSales
...
--Step 2: Calculate Rev Growth
SELECT
    [Sub Category],
    YearMonth,
    SUM(Revenue) AS Revenue,
    LAG(SUM(Revenue)) OVER (PARTITION BY [Sub Category] ORDER BY YearMonth) AS PreviousMonthRevenue,
    (SUM(Revenue) - LAG(SUM(Revenue)) OVER (PARTITION BY [Sub Category] ORDER BY YearMonth)) / LAG(SUM(Revenue)) OVER (PARTITION BY [Sub Category] ORDER BY YearMonth) * 100 AS MonthlyGrowthRate
FROM
    Bikecompany..BikeSales
GROUP BY
    [Sub Category], YearMonth
ORDER BY
    [Sub Category], YearMonth;
--This would give result for monthly fluctuation, not growth rate. Now we want to calculate growth rate (for eg sum of Jan-Jun15 vs Jan-Jun16)
